def dienas_mekletajs(sis_g, sis_m, sis_d, sis_n, dz_g, dz_m, dz_d):
    if dz_m < sis_m:
        return sis_n
    elif dz_m == sis_m:
        if dz_d < sis_d:
            ...
            return "Šodien ir jūsu dzimšanas diena!"
        else:
            return sis_n-1
    else:
        return sis_n-1
...
def datu_parbaude (gads_dz, menesis_dz, datums_dz, gads_sis, menesis_sis, datums_sis, diena_sis):
    pareizi_dati = True
    if (gads_dz>gads_sis) or (gads_dz==gads_sis and menesis_dz>menesis_sis) or (gads_dz==gads_sis and menesis_dz==menesis_sis and datums_dz>datums_sis):
        pareizi_dati = False
